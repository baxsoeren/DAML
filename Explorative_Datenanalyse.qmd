...
frequency_counts <- shopping_behavior %>%
  ...
  arrange(`Frequency of Purchases`)
...
shopping_behavior$`Frequency of Purchases` <- factor(shopping_behavior$`Frequency of Purchases`, levels = names(frequency_mapping))

unique_values <- unique(shopping_behavior$`Frequency of Purchases`)


ggplot(shopping_behavior, aes(x = `Frequency of Purchases`, fill = `Frequency of Purchases`)) +
  geom_bar(stat = "count") +
...
```{r}
unique_payment_methods <- unique(shopping_behavior$`Payment Method`)
...
```{r}
count_cash_payments <- sum(shopping_behavior$`Payment Method` %in% c("Bar", "Cash"), na.rm = TRUE)
...
payment_data <- raw_shopping_behavior %>%
  ...
  summarise(TotalAmount = sum(`Purchase Amount (USD)`)) %>%
  ungroup() %>%
  mutate(Percentage = TotalAmount / sum(TotalAmount) * 100)

payment_data %>%
  select(`Payment Method`, Percentage)
```
  </div>
  </div>
## These 4:
