...
## Random Forests (Modell 3, Fabian)
...
```
### Beurteilung
<p>Die Beurteilung für dieses Modell wird zusammen mit dem Modell 4 erfolgen, da beide Modelle die identischen Thesen untersuchen.</p>
:::
:::

## Neuronales Netz (Modell 4, Fabian)
...
<p>Wir schauen uns zunächst die Daten an, die wir für unser Modell verwenden wollen.</p>
...
set.seed(245)
data <- floor(0.8 * nrow(df))
data_rows <- sample(1:nrow(df), data)

train_data <- df[data_rows,]
#print(train_data)

test_data <- df[-data_rows,]
#print(test_data)
```

<p>Das Modell wird nun erstellt und die Vorhersagen getroffen. Zum tieferen Verständnis werden die Layer geplotet.</p>
...
<p>Da das Modell erstellt wurde, ist es nun an der Zeit das Modell zu evaluieren. Hierzu wird eine Konfusionsmatrix erstellt wobei die Zeile 0 "kein Express" und die Zeile 1 "Express" bedeutet..</p>

```{r}
pred <- predict(nn_model, test_data)
labels <- c("Express", "Standard")
prediction_label <- data.frame(max.col(pred)) %>%     
  mutate(pred = labels[max.col(pred)]) %>%
  select(pred) %>%
  unlist()

conf_matrix <- table(test_data$Express_Shipping_Used, prediction_label, dnn = c("Actual", "Predicted"))


heatmap_plot <- ggplot(data = as.data.frame(as.table(conf_matrix)), aes(x = Actual, y = Predicted, fill = Freq)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "orange", high = "blue") +
  theme_minimal() +
  labs(title = "Confusion Matrix", x = "Tatsächlich", y = "Vorhergesagt", fill = "Frequenz")
...
<p>Ein weiterer Wert zum Überprüfen der Performance ist die Accuracy.</p>
...
confusion_matrix = as.matrix((table(Actual_values = test_data$Express_Shipping_Used, Predicted_values = prediction_label)))
...
### Beurteilung
<p> Mittels der Modelle 3 und 4 wird mittels eines Machine-Learning und eines Random-Forestes Modells die Vorhersage der Verwendung des Shipping_Type "Express" ab einem Einkaufswert von 100€ analysiert. Beide Modelle beschäfitgen sich mit den selben Thesen, um einen Vergleich der Eignung herstellen zu können. Darüberhinaus arbeiten beide Modelle mit dem identischen Datensatz, um die Performancewerte bestmöglich miteinander vergleichen zu können.

Es stelle sich heraus, dass das Machine-Learning Modell mit einer Accuracy von 70 % eine wesentlich höhere Accuracy als das Random-Forests Modell (25%) hat.

Letzendlich lässt sich somit sagen, dass das Machine-Learning Modell für diese Analyse besser geeignet ist, als das Random-Forest Modell. </p>
:::
:::
