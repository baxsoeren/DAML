...
Zum Norden gehören folgende Staaten:
...
Zum Süden gehören folgende Staaten:
...
```{r}
 set.seed(1)
  mlp_cheese_fit <- mlp_cheese_mod |> fit(Express_Shipping_Used ~ Purchase_Amount, data = train_data)
  mlp_cheese_fit
```
...
```{r}
mlp_cheese_fit |> extract_fit_engine() |> summary()
```

Die Confusion Matrix wird nun als Heatmap dargestellt.

```{r}
pred_fb <- predict(mlp_cheese_fit, test_data)
...
Alle nicht explizit erwähnten Kapiterl wurden gemeinsam erarbeitet.
