...
```{r}
rf_model <- randomForest(Express_Shipping_Used ~ . - Purchase_Amount - Shipping_Type - Express_Shipping_Used, data = train_data_main)
```

Die Vorhersagen, welche von dem Modell getroffen werden, werden in einer Tabelle zusammengefasst.
```{r}
predictions_fb <- predict(rf_model, newdata = test_data_main)
prediction_table_fb <- data.frame(Express_Shipping_Used = test_data_main$Express_Shipping_Used, Predicted_Probability = predictions_fb)
...
confusion_matrix <- table(predictions_fb, test_data_main$Express_Shipping_Used)
...
nn_model <- neuralnet(
  ...
  data = train_data_main, 
  hidden = c(5),
  linear.output = TRUE
)

predictions_fb <- predict(nn_model, newdata = test_data_main)
```
...
```{r}
confusion_matrix <- table(rounded_predictions_fb, test_data_main$Express_Shipping_Used)
accuracy <- sum(diag(confusion_matrix)) / sum(confusion_matrix)
cat("Accuracy:", accuracy, "\n")
cat("Confusion Matrix:\n", confusion_matrix, "\n")
