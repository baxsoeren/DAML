...
| **Promo Code Used**        | chr | Zeigt an, ob bei ein Aktionscode oder ein Gutschein verwendet wurde |
...
**These 7: Im Norden werden wird mehr eingekaut, als im Süden.**
...
Zunächst ändern wir alle Spalten so um, dass die Leerzeichen durch Unterstriche ersetzt werden:
...
Zunächst schauen wir uns einmal die ersten 5 vorhandenen Orte an:

```{r}
shopping_behavior |> distinct(Location) |> head(5)
```

Wir sehen, dass es insgesamt `r nrow(shopping_behavior |> distinct(Location))` Orte gibt. Nun schauen wir uns die Häufigkeit der einzelnen Orte an:
...
Nun schauen wir uns an, wie viele Produkte im Norden und wie viele Produkte im Süden gekauft wurden:

```{r}
shopping_behavior |> filter(Location %in% c("Washington", "Oregon", "Idaho", "Montana", "Wyoming", "North Dakota", "South Dakota", "Nebraska", "Minnesota", "Iowa", "Missouri", "Wisconsin", "Illinois", "Michigan", "Indiana", "Ohio", "Kentucky", "New York", "Delaware", "Maryland", "Vermont", "West Virginia", "Connecticut",  "Maine", "Virginia", "Pennsylvania", "Alaska", "Massachusetts", "New Hampshire", "New Jersey", "Rhode Island")) |> count(Location)
```

```{r}
shopping_behavior |> filter(Location %in% c("California", "Nevada", "Utah", "Arizona", "Colorado", "New Mexico", "Kansas", "Oklahoma", "Texas", "Arkansas", "Louisiana", "Mississippi", "Tennessee", "Alabama", "North Carolina", "South Carolina", "Georgia", "Florida", "Hawaii")) |> count(Location)
```        
:::
:::
...
Nun erdtellen wir die Vorhersage und eine Vorhersagetabelle:
```{r}
...
Wir sehen, dass die Trefferwahrscheinlichkeit bei **71.7%** liegt und damit nur ganz okay ist. Insgesamt wurden **28.3%** richtig vorhergesagt.
...
Zum Schluss dieser Arbeit möchten wir diese nochmals kritisch betrachten. Hierzu gehen wir zunächst auf das verwendete Datenset und im Anschluss auf unsere Arbeitsweisen ein. Dabei hinterfragen wir besonders unser Vorgehen und die erzielten Ergebnisse. Das Datenset ist für für die Beantwortung unserer Thesen geeignet. Es umfasst einen Umfang von Rund 3700 Einträgen und ist somit ausreichend repräsentativ. Es gibt jedoch auch einige Kritikpunkte die wir an dieser Stelle anbringen möchten. 1. Kein Kaufdatum Bei den Datensätzen sind keine Kaufdaten hinterlegt. Dies wäre bei der Beantwortung der Thesen x und x von Vorteil gewesen, um die Thesen besser zu stützen oder sogar aufgrunddessen wiederlegen zu können. 2. Keine Umsätze über 100$ Es gibt keine Datensätze, welche einen Umsatz von über 100 $ aufweisen. 3. Keine Angabe darüber, ob Kunden mehrfach Käufe getätigt haben. Aus dem Datenset ist nicht ersichtlich, dass Kunden mehrfach Käufe getätigt haben könnten. Dies verfälscht wohlmöglich die Vorhersagen der Modelle. Es lässt sich nicht sicher sagen, dass jeder Kauf tatäschlich von einer anderen Person getätigt wurde. 4. Ungenaue Eingrenzung, ob etwas zum "Norden" oder "Süden" liegt. Die Unterteilung der USA in Nord und Süd basiert auf keinen zugrundeliegenden Informationen, sondern wurde "aus dem Gefühl" heraus vorgenommen. Dies kann zu einer Verfälschung der Ergebnisse führen, da die Einteilung nicht objektiv ist und Einträge in "Süd" strenggenommen zu "Nord" gehören könnten und anderes herum. Nun zu unserer Arbeitsweise. Allgemein sind wir strukturiert vorgegangen. Die Thesen wurden ausreichend gründlich analysiert und hinreichend belegt oder wiederlegt. Die Ergebnisse sind nachvollziehbar und die verwendeten Methoden sind angemessen. Hätte das Datenset tiefergreifende Informationen gehabt, hätten die Thesen noch tieferlegend analysiert werden können um so eine bessere Basis für das Erstellen der Modelle zu schaffen. Würden wir diese Arbeit erneut durchführen, würden wir ein anderes Datenset nehmen, welches die Informationen des Kaufdatums, der Umsätze von über 100$ beinhaltet und eine eindeutige Identifizierung des Käufers beinhaltet. So lassen sich die Vorhersagen der Modelle und Vorhersagen besser beurteilen.

## Ideen für weitere Analysen

Vorhersage von weiteren Variablen, wie z.B. die Anzahl der Bestellungen, die ein Kunde tätigt. Analyse des Ratings
