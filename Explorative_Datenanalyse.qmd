...
Nun werfen wir einen kurzen Blick auf die Datenstruktur des Datensets "shopping_behavior":

```{r}
knitr::kable(head(shopping_behavior), caption = "Das Spaltenformat des Datensatzes'shopping_behavior'")
```

Jede Zeile steht für einen Einkauf.
...
Im Datensatz gibt es **`r nrow(shopping_behavior)`** Instazen (Beobachtungen). Zudem gibt es keine Instanz, welche Werte ohne Angabe (NA) enthalten. Daher müssen wir keine fehlenden Variablen aufbereiten.
...
Wir sehen, dass es 50 Orte gibt. Nun schauen wir uns die Häufigkeit der Orte an:
