...
```{r}
shopping_behavior |> count(`Discount Applied`)
```
...
```{r}
shopping_behavior |> count(`Promo Code Used`)
```
...
Hierzu analysieren wir zunächst wie häufig der Wert "male" in der Spalte "Gender" vorkommt:

```{r}
shopping_behavior |> filter(Gender != "male") |> count(Gender)
```

Das Ergebnis der Auswertung ist, dass Personend des männlichen Geschlechtes 2652 Produkte kauften. Personen des weiblichen Geschlechts hingegen kauften nur 1248 Produkte.
...
Somit lässt sich die Hypothese belegen, dass im Winter mehr Kleidung gekauft wird als im Sommer, belegen. Im Winter werden `r nrow(shopping_behavior |> filter(Season == "Winter") |> count(Season))` Produkte gekauft. Im Sommer hingegen werden `r nrow(shopping_behavior |> filter(Season == "Summer") |> count(Season))` Produkte gekauft.
## These 7 (Sörn)
